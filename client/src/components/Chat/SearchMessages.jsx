...
function SearchMessages() {
  ...
  useEffect(()=>{
    if(serachTerm){
      setSearchMessages(messages.filter(message=>message.type==="text" && message.message.includes(serachTerm)))
    }else{
      setSearchMessages([])
    }
  },[serachTerm])
    ...
    <div className="overflow-auto custom-scrollbar h-full">
      <div className="flex items-center flex-col w-full">
        ...
        <span className="mt-10 text-secondary">
          {!serachTerm.length && `Search for messages with ${currentChatUser.name}`}
        </span>
      </div>
      <div className="flex justify-center h-full flex-col">
        {
          serachTerm.length>0 && !searchMessages.length && <span className=" text-secondary w-full flex justify-center">
            No messages found
          </span>
        }
